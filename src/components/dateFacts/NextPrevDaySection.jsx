...
export default function NextPrevMonthSection(props) {
    ...
    return (
        <section className="content-section no-p">
            <h2 className="section-heading-large">See Other Days!</h2>
            <div className="three-column-flex smaller-middle-div">
                {(props.day > 1) ? (
                    <Link to={`/date-facts/day/${props.day - 1}`} className="next-prev-column prev">
                        <h2 className="section-heading">Prev</h2>
                        <img className='arrow' src={arrowLeft} alt="prev" />¨
                <h2 className="large-number">{props.day - 1}</h2>
                    </Link>
                ) : (
                        <div></div>
                    )}
                <div></div>
                {props.day < 31 &&
                    <Link to={`/date-facts/day/${Number(props.day) + 1}`} className="next-prev-column next">
                        <h2 className="section-heading">Next</h2>
                        <img className='arrow' src={arrowRight} alt="next" />
                        <h2 className="large-number">{Number(props.day) + 1}</h2>
                    </Link>
                }
            </div>
        </section>
